...
{
    /// <summary>
    /// Interaction logic for BillingTabular.xaml
    /// </summary>
    public partial class BillingTabular : UserControl
    {
        ...
        public BillingTabular(string CurrentUserId1, string CurrentUserRole1, string CurrentUserPassword1, string CurrentUserName1, string CurrentUserEmployeeId1, string DTId1)
        {
            InitializeComponent();
        ...
        private async Task ReadDatabase()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(txtDtId.Text))
                {
                    ...
                    {
                        string url = string.Format("http://103.234.126.43:3080/api/instant/dt/{0}", txtDtId.Text);

                        string json = await web.DownloadStringTaskAsync(url);

                        List<BillingClass> all_data = JsonConvert.DeserializeObject<List<BillingClass>>(json);

                        // Filter data based on date range using the format "yyyy-MM-dd"
                        List<BillingClass> filtered_data = all_data
                            .Where(x => DateTime.ParseExact(x.record_date, "dd-MM-yyyy", CultureInfo.InvariantCulture) >= dtPickerStart.SelectedDate.Value
                                        && DateTime.ParseExact(x.record_date, "dd-MM-yyyy", CultureInfo.InvariantCulture) <= dtPickerEnd.SelectedDate.Value)
                            .ToList();

                        // Bind filtered data to DataGrid
                        _ = Dispatcher.InvokeAsync(() =>
                        {
                            datagridBilling.ItemsSource = filtered_data;
                        });
        ...
        private async void btnSendEmail_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                using (SmtpClient smtpClient = new SmtpClient("smtp.gmail.com"))
                {
                    ...
                    attachment.Name = "Billing_" + txtDtId.Text + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".xlsx";
        ...
        private void MenExportXls_Click(object sender, RoutedEventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog
            {
                ...
                FileName = "Billing_" + txtDtId.Text + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".xlsx"
        ...
        private void menExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog
            {
                ...
                FileName = "Billing_" + txtDtId.Text + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".csv"
        ...
        private void menExportPdf_Click(object sender, RoutedEventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog
            {
                ...
                FileName = "Billing_" + txtDtId.Text + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".pdf"
