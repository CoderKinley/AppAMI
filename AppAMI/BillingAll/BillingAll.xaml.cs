...
namespace AppAMI.BillingAll
{
    /// <summary>
    /// Interaction logic for BillingAll.xaml
    /// </summary>
    public partial class BillingAll : UserControl
    {
        ...
        public BillingAll(string CurrentUserId, string CurrentUserRole, string CurrentUserPassword, string CurrentUserName, string CurrentUserEmployeeId)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var apiResponseList = JsonConvert.DeserializeObject<List<BillingClass>>(jsonResult);
        ...
        private List<BillingClass> accumulatedBillingData = new List<BillingClass>();

        private async void GetBillingDataForStoredInfo(List<BillingClass> apiResponseList)
        {
            datagridBilling.ClearFilters();
            ...
            try
            {
                ...
                datagridBilling.ItemsSource = accumulatedBillingData;
        ...
        private async Task GetBillingDataForDistrictCode(string districtCode, string dtId)
        {
            ...
            try
            {
                string apiUrl = $"http://103.234.126.43:3080/api/instant/dt/{dtId}";
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        // Deserialize the JSON response to a List<BillingClass>
                        List<BillingClass> billingData = JsonConvert.DeserializeObject<List<BillingClass>>(jsonResult);

                        if (billingData != null && billingData.Any())
                        {
                            // Filter the billingData for the specific date range
                            List<BillingClass> filteredDateData = billingData
                                .Where(x => DateTime.TryParseExact(x.record_date, "dd-MM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime recordDate)
                                            && recordDate >= dtPickerStart.SelectedDate.Value
                                            && recordDate <= dtPickerEnd.SelectedDate.Value)
                                .ToList();

                            if (filteredDateData.Any())
                            {
                                // Append the filtered billing data to the accumulated data
                                accumulatedBillingData.AddRange(filteredDateData);
                            }
                            else
                            {
                                // Handle case where no data is found for the dtId within the date range
                                MessageBox.Show($"No billing data found for {dtId} within the selected date range.");
                            }
                        }
                        else
                        {
                            // Handle case where billingData is null or empty
        ...
        private void SaveDataToTemporaryStorage(string district, string esd, DateTime startDate, DateTime endDate, List<BillingClass> data)
        {
        ...
        private void BtnViewData_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button btnViewData && btnViewData.Tag is ViewDataButtonInfo buttonInfo)
            {
                ...
                List<BillingClass> temporaryData = GetTemporaryData(buttonInfo.District, buttonInfo.ESD, buttonInfo.StartDate, buttonInfo.EndDate);

                if (temporaryData != null)
                {
                    //MessageBox.Show($"Fetched Data Count: {temporaryData.Count}");
                    datagridBilling.ItemsSource = temporaryData;
                }
                else
                {
        ...
        private List<BillingClass> GetTemporaryData(string district, string esd, DateTime startDate, DateTime endDate)
        {
            string key = $"{district}_{esd}_{startDate.ToString("yyyyMMdd")}_{endDate.ToString("yyyyMMdd")}";

            if (temporaryStorage.TryGetValue(key, out List<BillingClass> temporaryData))
            {
                return temporaryData;
            }
            else
            {
                return new List<BillingClass>();
            }
        }

        private Dictionary<string, List<BillingClass>> temporaryStorage = new Dictionary<string, List<BillingClass>>();
        ...
        private void RefreshDataGrid()
        {
            if (stackCurrentData.Children.Count > 0 && stackCurrentData.Children[0] is StackPanel firstContentPanel)
            {
                ...
                {
                    List<BillingClass> temporaryData = GetTemporaryData(firstButtonInfo.District, firstButtonInfo.ESD, firstButtonInfo.StartDate, firstButtonInfo.EndDate);
        ...
        //        using (WebClient web = new WebClient())
        //        {
        //            // Set the fixed date range for the request
        //            string startDate = "19-09-2024";
        //            string endDate = "20-09-2024";
        //            string districtCode = "Bu";

        //            // Construct the URL with the district code and the fixed date range
        //            string url = string.Format("http://103.234.126.43:3080/api/instant/district/{0}/{1}/{2}", districtCode, startDate, endDate);
        ...
        //            List<BillingClass> allData = JsonConvert.DeserializeObject<List<BillingClass>>(json);
        ...
        //            // No filtering needed, as data is already fetched between the specified dates
        //            await Dispatcher.InvokeAsync(() =>
        //            {
        //                // Update DataGrid with the fetched data
        //                datagridBilling.ItemsSource = allData;

        //                // Calculate total counts
        //                int totalInstantHisCount = 0;
        //                int totalUniqueDtCount = 0;

        //                HashSet<string> uniqueDtIds = new HashSet<string>();

        //                if (datagridBilling.View?.Records != null)
        //                {
        //                    foreach (var item in datagridBilling.View.Records)
        //                    {
        //                        var rowData = item.Data as BillingClass;

        //                        if (!string.IsNullOrEmpty(rowData?.dt_id))
        //                        {
        //                            totalInstantHisCount++;
        ...
        //                lblTotalInstantHisCount.Content = totalInstantHisCount;
        ...
        private async Task ReadDatabase()
        {
            ...
            if (!string.IsNullOrWhiteSpace(dtPickerStart.Text) || !string.IsNullOrWhiteSpace(dtPickerEnd.Text))
            {
                try
                {
                    using (WebClient web = new WebClient())
                    {
                        string startDate = dtPickerStart.SelectedDate?.ToString("dd-MM-yyyy");
                        string endDate = dtPickerEnd.SelectedDate?.ToString("dd-MM-yyyy");

                        string url = string.Format("http://103.234.126.43:3080/api/instant/district/{0}/{1}/{2}", selectedDistrict, startDate, endDate);
                        ...
                        List<BillingClass> filteredDataPlace = new List<BillingClass>();
                        ...
                        await Dispatcher.InvokeAsync(() =>
                        {
                            datagridBilling.ItemsSource = filteredDataPlace;
        ...
        private void MenExportXls_Click(object sender, RoutedEventArgs e)
        {
            ...
            var excelEngine = datagridBilling.ExportToExcel(datagridBilling.View, options);
            ...
            SaveFileDialog sfd = new SaveFileDialog
            {
                ...
                FileName = "Billing_" +selectedDistrict +"_" + selectedEsd + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".xlsx"
        ...
        private void menExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            SaveFileDialog sfd = new SaveFileDialog
            {
                ...
                FileName = "Billing_" + selectedEsd + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".csv"
        ...
        {
            var document = datagridBilling.ExportToPdf();
            SaveFileDialog sfd = new SaveFileDialog
            {
                Filter = "PDF Files(*.pdf)|*.pdf",
                FileName = "Billing_" + selectedEsd + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".pdf"
        ...
        private async void btnSendEmail_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                MailMessage message2 = new MailMessage
                {
                    ...
                    Body = "<html><body> Kindly find the attached Billing Data</body></html>",
                    IsBodyHtml = true
                };
                ...
                using (SmtpClient smtpClient = new SmtpClient("smtp.gmail.com"))
                {
                    ...
                    attachment.Name = "Billing_" + selectedEsd + "_From_" + dtPickerStart.Text + "_To_" + dtPickerEnd.Text + ".xlsx";
                    ...
                    MessageBox.Show("Billing Data Successfully Sent", "Success Notification", MessageBoxButton.OK, MessageBoxImage.Information);
